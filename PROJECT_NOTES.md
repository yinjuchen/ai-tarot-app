...
- 已上線:https://ai-tarot-app-jjey.vercel.app/
- `/api/tarot` 有簡易 per-IP rate limiting(每分鐘 5 次),防止公開 API 被亂打燒 OpenAI 額度
- README.md 已從 Vite 預設模板重寫成正式專案說明(功能、技術棧、安裝方式)
...
**問題輸入框 UX 微調(已修復,2026-07-12)**
- 按 Enter 現在會直接觸發抽牌(原本只有點按鈕才有反應)
- 抽牌一開始就清空輸入框(問題已在當下被記住並傳給 API),方便馬上打下一個問題

## 面試 / 展示準備

使用者問這個專案能不能拿去面試用,盤點後優先處理了兩件事:

1. **`/api/tarot` 加上 per-IP rate limiting**(已完成,2026-07-13)—— 這支 API 完全公開、沒有登入機制,原本任何人狂打就能燒 OpenAI 額度。現在用進程內記憶體限制每個 IP 每分鐘最多 5 次,超過回 429,前端顯示對應的友善訊息。侷限:同一 IP 的計數只在單一 serverless instance 存活期間有效,instance 之間不共用 —— 足夠擋掉單純迴圈亂打,但不是嚴謹的分散式限流,正式產品應該換 Upstash Redis / Vercel KV。
2. **README.md 重寫**(已完成,2026-07-13)—— 原本還是 Vite 預設模板文字,完全沒說明專案在做什麼。重寫成含功能說明、技術棧、安裝步驟、環境變數表、專案結構的正式 README。截圖留了 placeholder 註解,因為這次環境沒辦法把截圖存成檔案放進 repo,需要使用者自己截圖補上。

**還沒做、面試官可能會問的**
- [ ] 完全沒有測試(unit test / API 基本測試)
- [ ] `App.jsx` UI 跟商業邏輯混在一起,可以拆 hook 讓結構更清楚
- [ ] README 少一張實際畫面截圖(需要使用者自己補)

## 下一步(未決定)
- [ ] 釐清並清理 Vercel 上重複的三個專案,只留一個當正式站(使用者已決定留 `ai-tarot-app-jjey`,尚未實際刪除)
- [ ] 視覺打磨:三張卡片間距、手機版排版是否需要進一步調整
- [ ] 考慮「三張牌共用一個貫穿意象」是否要加入
- [ ] 考慮是否保留「單抽一張」作為次要模式,或完全以三張牌排陣為主
- [ ] 考慮加真正的文字轉語音朗讀功能(使用者一度誤會「占卜師語氣」是真的聲音,討論後決定先不做,之後有興趣可評估 OpenAI TTS 或 Web Speech API)
